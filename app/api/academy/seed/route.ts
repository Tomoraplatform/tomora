...
/**
 * Temporary, token-gated seeding endpoint used once to load the first course
 * (media lives locally; Vercel env vars are sensitive so seeding must run
 * server-side). Guarded by ACADEMY_SEED_TOKEN; returns 404 without it.
 * Remove this route after the first course is seeded.
 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (body.action === "status") {
      const { error } = await admin.from("academy_courses").select("id").limit(1);
    ...
    if (body.action === "create-course") {
      ...
      const { data: course, error: cErr } = await admin.from("academy_courses")
        .insert({ title: c.title, slug: c.slug, short_description: c.short_description, price: c.price, thumbnail_url: c.thumbnail_url || null, is_published: false, sort_order: 1 })
        .select("id").single();
