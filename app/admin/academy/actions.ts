...
export async function updateCourse(id: string, patch: Partial<{ title: string; slug: string; short_description: string; thumbnail_url: string; price: number; compare_price: number; is_published: boolean; sort_order: number; is_coming_soon: boolean; coming_soon_lessons: number }>): Promise<R> {
  try {
    const admin = await guard();
