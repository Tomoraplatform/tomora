...
import {
  ArrowLeft, CheckCircle2, Circle, ChevronDown, ChevronLeft, ChevronRight,
  Loader2, PlayCircle, FileText, Link2,
} from "lucide-react";
import { SlideViewer } from "@/components/academy/slide-viewer";
import { setCreatorLessonComplete } from "@/app/c/progress-actions";
import type { CreatorCourseWithContent } from "@/lib/creator/db";

type Lesson = CreatorCourseWithContent["modules"][number]["lessons"][number];
type Media = { type: string; embedUrl?: string; rawUrl?: string; fileUrl?: string; title?: string; description?: string };

/**
 * Player for creator courses: mirrors the Tomora Academy experience with
 * support for uploaded video, embedded links and PDF lessons.
 */
export function CreatorPlayer({ course, creatorSlug, completedLessonIds, studentName, authorName }: {
  ...
  const [active, setActive] = useState<Lesson | undefined>(
    flat.find((l) => !completedLessonIds.includes(l.id)) || flat[0]
  );
  ...
  const load = useCallback(async (lesson: Lesson) => {
    setLoading(true); setMedia(null); setError(null);
    try {
      const res = await fetch(`/api/creator/lesson/${lesson.id}`, { cache: "no-store" });
  ...
  async function toggleComplete(lesson: Lesson, value?: boolean) {
    ...
    setDone((s) => { const n = new Set(s); if (target) n.add(lesson.id); else n.delete(lesson.id); return n; });
    await setCreatorLessonComplete(lesson.id, target);
  }
      ...
      <div className="flex flex-1 flex-col lg:flex-row">
        <main className="min-w-0 flex-1">
          <div className="bg-black">
            {loading && <div className="flex aspect-video items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-white/40" /></div>}

            {!loading && error && (
              <div className="flex aspect-video flex-col items-center justify-center gap-2 px-6 text-center">
                <p className="text-sm text-white/70">{error}</p>
                {active && <button onClick={() => load(active)} className="rounded-md bg-white/10 px-4 py-2 text-xs font-semibold">Try again</button>}
              </div>
            )}

            {!loading && !error && media?.type === "link" && media.embedUrl && (
              <iframe src={media.embedUrl} title={media.title || "Lesson"} allowFullScreen className="aspect-video w-full border-0" />
            )}

            {!loading && !error && media?.type === "video" && media.fileUrl && (
              <video key={media.fileUrl} src={media.fileUrl} controls controlsList="nodownload noremoteplayback"
                disablePictureInPicture onContextMenu={(e) => e.preventDefault()}
                onEnded={() => active && toggleComplete(active, true)}
                className="aspect-video w-full" playsInline autoPlay />
            )}

            {!loading && !error && media?.type === "pdf" && media.fileUrl && (
              <div className="bg-white"><SlideViewer url={media.fileUrl} /></div>
            )}

            {!loading && !error && media && !media.embedUrl && !media.fileUrl && (
          ...
          <div className="px-5 py-5">
            ...
            {active?.description && <p className="mt-2 text-sm text-white/60">{active.description}</p>}

            <div className="mt-5 flex flex-wrap items-center gap-2">
              {active && (
                <button onClick={() => toggleComplete(active)}
                  className={`inline-flex items-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold ${done.has(active.id) ? "bg-emerald-600 text-white" : "bg-white text-[#101319]"}`}>
                  <CheckCircle2 className="h-4 w-4" /> {done.has(active.id) ? "Completed" : "Mark as complete"}
                </button>
              )}
            ...
            <p className="mt-6 text-xs text-white/30">Signed in as {studentName} · Course by {authorName}</p>
          </div>
        </main>
            ...
            {course.modules.map((m, mi) => {
              ...
              return (
                <div key={m.id} className="border-b border-white/5">
                  <button onClick={() => setOpenModules((s) => { const n = new Set(s); if (isOpen) n.delete(m.id); else n.add(m.id); return n; })}
                    className="flex w-full items-center justify-between gap-3 px-5 py-3.5 text-left">
                    <span className="min-w-0">
                      <span className="block truncate text-sm font-semibold">Module {mi + 1}: {m.title}</span>
                  ...
                  {isOpen && m.lessons.map((l) => {
                    ...
                    const Icon = l.lesson_type === "pdf" ? FileText : l.lesson_type === "link" ? Link2 : PlayCircle;
                    return (
                      <button key={l.id} onClick={() => { setActive(l); setSidebarOpen(false); }}
                        ...
                        {done.has(l.id) ? <CheckCircle2 className="h-4 w-4 shrink-0 text-emerald-500" /> : <Circle className="h-4 w-4 shrink-0 text-white/25" />}
